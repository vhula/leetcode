...
public class TopKFrequentElements {
    ...
    @Solution(
            timeComplexity = @TimeComplexity(value = Complexity.N_LOG_N, details = "N is the number of unique elements in nums"),
            spaceComplexity = @SpaceComplexity(value = Complexity.LINEAR, details = "to store the frequency map"),
    ...
    public int[] topKFrequentUsingSorting(int[] nums, int k) {
        ...
        List<int[]> sortedByFrequency = frequency.entrySet()
                .stream()
                .map(e -> new int[]{e.getValue(), e.getKey()})
                .sorted(Comparator.comparingInt(a -> ((int[]) a)[0]).reversed())
                .toList();
        return sortedByFrequency.subList(0, k).stream().mapToInt(a -> a[1]).toArray();
    }

    @Solution(
            timeComplexity = @TimeComplexity(customValue = "O(N log K)", details = "N is the number of elements in nums. K is the input parameter k"),
            spaceComplexity = @SpaceComplexity(customValue = "O(N + K)", details = "to store the frequency map and the heap"),
